...
//▼fujiwara
						//数値チェック
						if(!(elements.eq(0).val() >= -9999 && elements.eq(0).val() <= 9999)){
							error_span( elements.eq(0), dt_name, '正しい数値で入力', true );
							error++;
							scroll_point = compare_method( scroll_point, elements.eq(0).offset().top );
						}else{
							error_span( elements.eq(0), dt_name, '', false );
...
//▼fujiwara			
//iDeCoチェック
		var element   = $( 'select#in_ideco' );
		
		if ( element.val() > 2.3 && $('input[name="workstyle"]:checked').val() == 0) {
			error_span( element.eq(0), '掛金', '上限(2.3万円)以下の金額で設定', true );
			error++;
			scroll_point = compare_method( scroll_point, element.eq(0).offset().top );
		}else{
			error_span( element.eq(0), '掛金', '', false );
		}
//▲fujiwara
